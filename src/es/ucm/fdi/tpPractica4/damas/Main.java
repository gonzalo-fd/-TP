...
public class Main {
	 ...
	 */
	enum GameInfo {
		CONNECTN("cn", "ConnectN"), TicTacToe("ttt", "Tic-Tac-Toe"), AdvancedTicTacToe("attt", "Advanced Tic-Tac-Toe"), Damas("damas", "damas");
	 ...
	 * 
	 * <p>
	 * Numero de obstaculos proporcionadas con la opcion -o, o {@code null} si no
	...
	private static void parseArgs(String[] args) {
		...
		cmdLineOptions.addOption(constructObstacles()); // -o or --obstacles
	 ...
	 */
	private static Option constructObstacles(){
		Option obstacles = new Option("o", "obstacles", true, "Creates Obstacles");;
		return obstacles;	
	}
	...
	private static void parseObsOption(CommandLine line) throws ParseException {
		...
		if (obsVal != null) {
			try {
				String[] obs = obsVal.split(" ");
				if (obs.length == 1) {
					obstacles = Integer.parseInt(obs[0]);
				} else {
